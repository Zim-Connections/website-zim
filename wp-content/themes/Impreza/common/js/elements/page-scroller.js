...
;( function( $, undefined ) {
	...
	$us.PageScroller.prototype = {
		...
		_destroy: function() {
			...
			_document.removeEventListener( 'mousewheel', that._events.mouseWheelHandler );
			_document.removeEventListener( 'DOMMouseScroll', that._events.mouseWheelHandler );
			_document.removeEventListener( 'MozMousePixelScroll', that._events.mouseWheelHandler );
		...
		_mouseWheelHandler: function( e ) {
			...
			if ( isAccelerating ) {
				...
				if ( that.sections[ target ] === _undefined ) {
					return;
				}
		...
		_attachEvents: function() {
			...
			if ( $us.$window.width() > this.disableWidth && $us.mobileNavOpened <= 0 && ( ! $us.$html.hasClass( 'cloverlay_fixed' ) ) ) {
				_document.addEventListener( 'mousewheel', that._events.mouseWheelHandler, { passive: false } );
				_document.addEventListener( 'DOMMouseScroll', that._events.mouseWheelHandler, { passive: false } );
				_document.addEventListener( 'MozMousePixelScroll', that._events.mouseWheelHandler, { passive: false } );
				...
				if ( $.isMobile || this.isTouch ) {
					...
					$us.$canvas.on( 'touchmove', function( event ) {
						...
						if ( Math.abs( this.touchStartY - this.touchEndY ) > ( $us.$window.height() / 50 ) ) {
							...
							if ( this.sections[ target ] === _undefined ) {
								return;
							}
