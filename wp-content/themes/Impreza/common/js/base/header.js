...
! function( $, undefined ) {
	...
	function USHeader( settings ) {
		...
		self.pos = self.$container.usMod( 'pos' ); // possible values: fixed|static
		self.bg = self.$container.usMod( 'bg' ); // possible values: solid|transparent
		self.shadow = self.$container.usMod( 'shadow' ); // possible values: none|thin|wide
		self.orientation = $us.$body.usMod( 'header' ); // possible values: hor|ver
	...
	$.extend( USHeader.prototype, $us.mixins.Events, {
		...
		_events: {
			...
			scroll: function() {
				...
				if (
					...
					if ( self.isStickyEnabled() ) {
						...
						if ( isSticky != self.isSticky() ) {
							/**
							 * Fixed to avoid jitter
							 * @link https://scroll.us-themes.net/8-2-regular-content-initial-header-position/
							 */
							if ( ( scrollTop % 1 ) === 0 ) {
								return;
							}
			...
			resize: function() {
				...
				self.setState( newState || 'default' );
